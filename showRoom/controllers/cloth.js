...
exports.addCloth = async function (req, res, next){
	...
	let clothPhoto
	if(PhotoFile){
		clothPhoto = config.Customer.imageurl+PhotoFile.destination.substr(8,PhotoFile.destination.length)+'/'+PhotoFile.filename;
	}else{
...
//		cloth1Size :Boolean(req.body.cloth1Size/1), //방법1
//		cloth1Size :!!parseInt(req.body.cloth1Size), //방법2
	};
	...
	let [cloth_CategoryInsert,cloth_shopInsert] = 
	await Promise.all([
		dbModel.dbOverlap2(cloth.addCloth.cloth_Category,cloth_catagory),
		dbModel.dbOverlap2(cloth.addCloth.cloth_shop,cloth_shopName)
	]); 
...
exports.editCloth = async function (req, res, next){
	let shopInfo = await jsonWebToken.TokenCheck(req.body.shopToken); //토큰확인
	let clothNumber = req.params.clothNumber;
	let clothInfo1 = {};
	let PhotoFile=req.files[0];
	if(PhotoFile){
		clothInfo1.clothPhoto = config.Customer.imageurl+PhotoFile.destination.substr(8, PhotoFile.destination.length)+'/'+PhotoFile.filename
	}
	...
	if(PhotoFile){
		...
		if(clothData.length == 0){
			return res.status(400).send('clothPhoto not exist!!!!!!!!!!!!');
  			...
  			var deleteData = 'public/'+clothData[0].clothPhoto.substr(15,clothData[0].clothPhoto.length);
		    fs.unlink(deleteData,function(err){
		        if (err){
		        	console.log('파일 삭제 실패 :',err);
		        	res.status(400).send('파일 삭제 실패 !!!!!!!!!!!!');
		        }else{
		        	console.log('clothPhoto 수정완료');
		        }
		    });
		}
	}
...
exports.deleteCloth = async function (req, res, next) {
	...
	if(clothData.length == 0){
		...
		let deleteData = 'public/'+clothData[0].clothPhoto.substr(15,clothData[0].clothPhoto.length);//아이디들어갈시 28
	    fs.unlink(deleteData,function(err){
	        if (err){
	        	console.log('파일 삭제 실패 :',err);
	        	res.status(400).send('파일 삭제 실패 !!!!!!!!!!!!');
	...
	let updateData = { $pull: { codyClothNumber : clothNumber , codyCategoryCode : categoryCode }};
...
exports.detailCloth = async  function (req, res, next){
	let shopInfo = await jsonWebToken.TokenCheck(req.body.shopToken); 
	let clothNumber = req.params.clothNumber;
	let clohData = await dbModel.dbOverlap2(cloth.showDeTailCloth,clothNumber);
...
exports.showClothTypeList = async function (req, res, next) {
	...
	let clohData = await dbModel.dbOverlap2(cloth.showClothTypeList,[shopInfo.ObjectId,clothType]);
	if(clohData == 0){
    	output.msg = 'cloth data not exist';
    	output.shopToken = shopInfo.userToken;
    	res.json(output);
	}else{
		var clothAllData=changeData(clohData);
	  	output.msg = 'cloth show success';
	  	output.shopToken = shopInfo.userToken;
	  	output.data = clothAllData;
	  	res.json(output);
...
exports.showClothList = async function (req, res, next) {
	...
	if(clohData == 0){
		...
		for(var i = 0; i< clohData.length; i++){ //코디에 옷 개수만큼 
			var temp = new Object();
...
//	favoriteInsert 부분 
	...
	//=============================================================================
	let showCloth = await dbModel.dbOverlap2(category.showCloth,[clothNumber]);
	let clothData = [];
	let temp = new Object();

	for(var i = 0 ; i<userInfo.userClothMybag.length; i++){
		if(clothNumber == userInfo.userClothMybag[i]){
			temp.clothIsStar= true;
		}else{
			temp.clothIsStar= false;
		}
	...
	temp.clothNumber = showCloth[0].clothNumber;
	temp.clothName = showCloth[0].clothName;
	temp.clothPhoto = showCloth[0].clothPhoto;
	temp.clothUrl = showCloth[0].clothUrl;
	clothData.push(temp);
		
  	output.msg = 'cloth detail show success ';
...
exports.userClothMybag = async function (req, res, next){
	...
	output.msg ='cloth 즐겨찾기 success';
	output.data = userTokenInfo.UserToken;
	res.json(output);
}	
...
// 		else {
// 			var f = files.pop();
// 			var fstrArray = f.split('/');
// 			var fdelete='public/'+fstrArray[3]+'/'+fstrArray[4]+'/'+fstrArray[5]+'/'+fstrArray[6];
// 			fs.unlink(fdelete, function(err) {
// 				if(err)
